	...
	m_LastFrameBufferDirtyIndex(0)
{
	...
	ASSERT(m_PixelSize.X != 0, "PixelSize.X must be greater than zero");
	ASSERT(m_PixelSize.Y != 0, "PixelSize.Y must be greater than zero");
}
...
void ILI9341_HAL::DrawPixel(Point Position, Color Color)
{
...
void ILI9341_HAL::PaintPixel(int16 X, int16 Y, uint16 R5G6B5, uint8 Alpha)
{
	...
	m_FrameBufferDirty[index / m_FrameBufferChunkSize] = true;
}

void ILI9341_HAL::InitializeSPI(GPIOPins SCLK, GPIOPins MOSI, GPIOPins NSS, GPIOPins DC, GPIOPins RST)
{
...
void ILI9341_HAL::InitDriver(Orientations Orientation)
{
	m_RST.Write(0);
	m_HAL->Delay(100);
...
uint8 ILI9341_HAL::SetOrientationAndGetTheRotationBits(Orientations Orientation)
{
	...
	{
		m_Dimension = { m_PixelSize.Y, m_PixelSize.X };
		rotationBits |= ili_my;
	}
	break;
...
void ILI9341_HAL::OnDMATransmissionCompleted(void* Context, daisy::SpiHandle::Result Result)
{
